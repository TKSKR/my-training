...
print(f"Сейчас - часов {datetime.now().hour}",
      f"минут: {datetime.now().minute}",
      f"Дата: {datetime.now().day},"
      f"Месяц: {datetime.now().month}, "
      f"Год: {datetime.now().year}")

# from time import sleep
# print('Привет! Я робот, с которым можно обсудить повышение вашей зарплаты.')
# people = {"Короткова": "Светлана",
#         "Коротков" : ["Сергей", "Кирилл", "Даниил", "Иван"],
#           "Кочетков": "Сергей",}
#
# while True:
...
#     people = people.upper()
#     if surname in [people.lower() for people in people]:
#         print(f"Приятно познакомиться, {people.upper([surname])}")
#         break
#     else:
...
people = {
    "Иванов": ["Иван", "Мария"],
    "Петров": ["Андрей", "Ольга"],
    "Сидоров": ["Иван", "Наталья"]
}

user_input = input("Введите имя или фамилию человека: ")

for key, value in people.items():
    if user_input in value:
        print(f"Этот {key} {user_input} находится в списке.")
...
#
# employee_name = ["Сергей", "Света", "Светлана", "Сережа", "Серёжа"]
