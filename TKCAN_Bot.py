...
async def cmd_login(message: types.Message):
    ...
    @dp.message()
    async def get_email(msg: types.Message):
        user_emails[msg.from_user.id] = msg.text
        await msg.answer("Теперь введите ваш пароль:")

        @dp.message()
        async def get_password(msg2: types.Message):
            email = user_emails.get(msg2.from_user.id)
            password = msg2.text
            token = await authenticate_user(email, password)

            if token:
                user_tokens[msg2.from_user.id] = token
                await msg2.answer("Вы успешно авторизованы! Используйте команду /all для списка ТС.")
            else:
                await msg2.answer("Ошибка авторизации. Проверьте введённые данные.")
