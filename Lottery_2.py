...
class NonLosingLottery:
    ...
    def conduct_draw(self, draw_number):
        ...
        # Randomly exclude a percentage of players
        excluded_count = max(1, round(len(self.participants) * random.uniform(0.001, 0.05)))
        excluded_players = random.sample(list(self.participants.keys()), excluded_count)
        for ticket in excluded_players:
            del self.participants[ticket]

        # Add new participants to maintain the number of sold tickets
        new_participants = {}
        for i in range(len(self.participants) + 1, self.tickets_sold + 1):
            ticket_id = f"Билет_{i}"
            new_participants[ticket_id] = 0
        self.participants.update(new_participants)

        # Process additional prizes
        ...
        if additional_prizes > 0:
            self.additional_prize_draws.append((draw_number, additional_prizes))
    ...
    def get_status(self):
        ...
        if self.additional_prize_draws:
            status_lines.append("Розыгрыши с дополнительными призами:")
            for draw, count in self.additional_prize_draws:
                status_lines.append(f"  Розыгрыш {draw}: {count} дополнительных призов")
        else:
...
class LotteryApp:
    def __init__(self, root):
        ...
        self.search_button.grid(row=5, column=0, sticky=tk.W)
    ...
    def run_lottery(self):
        try:
            ...
            if self.lottery.additional_prize_draws:
                additional_prize_info = [
                    "Розыгрыши с дополнительными призами:"
                ]
                for draw, count in self.lottery.additional_prize_draws:
                    additional_prize_info.append(f"  Розыгрыш {draw}: {count} дополнительных призов")
                messagebox.showinfo("Информация о дополнительных призах", "\n".join(additional_prize_info))
            else:
